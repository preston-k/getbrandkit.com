...
var printTasks = function(dir, file) {
  ...
  if (Object.keys(sake.options).length > 1) {
    console.log();
    results = [];
    for (k in sake.options) {
      if (!hasProp.call(sake.options, k)) continue;
      v = sake.options[k];
      if (isFunction$3(v)) {
        continue;
      }
...
var run$1 = run = function() {
  var argv, dir, err, file, i, len, task, tasks;
...
// Bail if missing task
  ...
  // Let's drink
  return sake__default.invoke(tasks, argv, function(err) {
    if (err != null) {
      console.error(err);
      return process.exit(1);
    }
