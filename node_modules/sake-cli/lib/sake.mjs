...
import sake, { options, tasks } from 'sake-core';
...
// src/cache.coffee
var cache, cacheDir, cachePath, findNodeModules, load, mkdirp, readOrigin, requireCached, write;

// Find local node_modules relative to current project
findNodeModules = function(dir) {
  var p;
  while (true) {
...
cacheDir = function(dir) {
  return path.join(findNodeModules(dir), '.sake');
};
...
readOrigin = function(dir) {
  try {
    return fs.readFileSync(path.join(cacheDir(dir), 'origin'), 'utf8');
  } catch (error) {
    return dir;
  }
};
...
write = function(dir, code) {
  ...
  return fs.writeFileSync(path.join(cacheDir(dir), 'origin'), dir, 'utf8');
};
...
requireCached = function(dir) {
  ...
  if (err != null) {
    throw err; // Throw if failed to require Sakefile
  }
};
...
load = function(dir, file) {
  ...
  if (cached.mtime > source.mtime && dir === (readOrigin(dir))) {
    requireCached(dir);
    return true;
  } else {
    return false;
...
// src/load.coffee
var load$1, loadCakefile, loadSakefile, loadSakefileTs;
...
loadSakefileTs = function(dir, file) {
  ...
  exec.sync(`tsc --types sake-core --target ES6 --outFile ${cache$1.path(dir)} Sakefile.ts`);
  // Load from cache
  return cache$1.require(dir);
};
...
// node_modules/executive/lib/executive.es6.js
...
var sync$1 = sync = function(cmd, opts, cb) {
  ...
  [cmd, args, opts] = parse$4(cmd, opts);
  ({pid, output, stdout, stderr, status, signal, error} = spawnSync(cmd, args, {
    cwd: opts.cwd,
    input: opts.input,
    stdio: (ref = opts.stdio) != null ? ref : [0, 'pipe', 'pipe'],
    env: opts.env,
...
var isFunction$3 = isFunction$2 = function(value) {
  var str;
  if (typeof window !== 'undefined' && value === window.alert) {
    return true;
  }
...
var printTasks = function(dir, file) {
  var desc, filePath, i, k, len, name, names, ref, relative, results, spaces, switches, task, v;
  ...
  if (Object.keys(options).length > 1) {
    ...
    for (k in options) {
      ...
      spaces = spaces > 0 ? Array(spaces + 1).join(' ') : '';
      results.push(console.log(`  ${switches}${spaces} ${(ref = v.description) != null ? ref : ''}`));
    }
    return results;
  }
};
...
var run$1 = run = function() {
  var argv, dir, err, file, i, len, task, tasks$$1;
  // Save record of original directory
  global.__originalDirname = fs.realpathSync('.');
  // Process arguments
  argv = yargs.argv;
  tasks$$1 = argv._.slice(0); // Create a copy of tasks to execute
...
// Bail if missing task
  for (i = 0, len = tasks$$1.length; i < len; i++) {
    task = tasks$$1[i];
    if (sake.tasks[task] == null) {
      missingTask(task);
    }
  }
  // Let's drink
  return sake.invoke(tasks$$1, argv, function(err) {
    if (err != null) {
      console.error(err);
      return process.exit(1);
    }
